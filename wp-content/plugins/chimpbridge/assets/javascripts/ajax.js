...
function build_default_name_email_array() {
	...
	.each( function( ) {
	    ...
	    if ( chimpbridge_ajax_localized_strings["Select a list"] === list_id ) {
		return true;
	    }
...
function populate_select( selectField, refreshButton, action, listID ) {
	...
	jQuery.post( chimpbridgeAjax.url, data, function( response ) {
		...
		if ( response.success ) {
			...
			selectField
				.empty()
				.append( '<option disabled selected>' + chimpbridge_ajax_localized_strings["Select a " + ( listID ? "segment" : "list" )] + '</option>' )
				.prop( 'disabled', false );

			if ( listID ) {
				selectField
					.append( '<option value="send-to-all">' + chimpbridge_ajax_localized_strings["Send to entire list"] + '</option>' );
			}
...
function update_segment_and_list_confirmation_message() {
    ...
    if ( segmentName == chimpbridge_ajax_localized_strings["Send to entire list"] ) {
        jQuery('#chimpbridge-verify-segment').html( 'entire' );
        return;
    }

    if ( segmentName != chimpbridge_ajax_localized_strings["No List Selected"] && segmentName != chimpbridge_ajax_localized_strings["Select a segment"] && segmentName != chimpbridge_ajax_localized_strings["This list has no segments"] ) {
		...
		if ( jQuery('#chimpbridge-refresh-lists').hasClass('disabled') ) {
			...
			jQuery.post( chimpbridgeAjax.url, data, function( response ) {
				if ( response.success ) {
					...
					disable_select_field( jQuery('#chimpbridge-select-segments'), chimpbridge_ajax_localized_strings["No List Selected"] );
					disable_refresh_button( jQuery('#chimpbridge-refresh-segments') );
